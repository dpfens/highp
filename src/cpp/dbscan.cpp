...
namespace density {
    ...
    private:
        ...
        void expand_cluster(std::vector<std::vector<T> > &data, const int index, std::vector<int> index_neighbors, std::vector<int> &clusters, int cluster_id) {
        ...
        std::vector<int> neighbors(std::vector<std::vector<T> > &data, std::vector<T> point) {
        ...
        public:
            DBSCAN(const long double epsilon, const long int min_points, long double (* distance_func)(std::vector<T>, std::vector<T>)) {
                assert(epsilon > 0);
                assert(min_points > 0);
                m_epsilon = epsilon;
            ...
            std::vector<int> predict(std::vector<std::vector<T> > &data) {
                ...
                for(auto it = data.begin(); it != data.end(); ++it) {
                    ...
                    std::vector<int> point_neighbors = neighbors(data, *it);
